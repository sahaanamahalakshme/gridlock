...
def run():
    ...
    pairs = session.query(
        distinct(Event.police_station), Event.event_cause
    ).all()
    ...
    print(f"  (raw CSV had {EXPECTED_TOTAL_PAIRS} - small drop is expected, from the 2 dropped rows)")
    print(f"low_precedent pairs (count<5): {low_precedent_count}  "
          f"({low_precedent_count/len(pairs):.1%})")
    print(f"  (reference: {EXPECTED_LOW_PRECEDENT_PAIRS}/{EXPECTED_TOTAL_PAIRS} = 48.7% from raw CSV)")
    print(f"tier breakdown: {tier_breakdown}")
    print(f"strongest precedent: {strongest[0]} + {strongest[1]} = {strongest[2]} prior events")
