...
validate_tune.py
================
Reads data/splits/val.csv + trained models from models/ and:
  1. Reports val accuracy/F1 for the default LR model.
  2. Sweeps C values for Logistic Regression to find the best.
  3. Also tries a LinearSVC as a comparison.
  4. Saves the BEST cause_classifier and priority_classifier back to
     models/ (overwriting the originals if the tuned version is better).
  5. Saves a val_report.txt to reports/.

The embedding model is NOT re-run for val — we embed val.csv once and
cache it in models/val_embeddings.npy.

Usage:
    python src/validate_tune.py
"""
...
# C values to sweep for Logistic Regression
...
def try_svc(X_train, y_train, X_val, y_val, label: str):
    """Train LinearSVC + Platt calibration (needed for predict_proba)."""
...
def main():
    ...
    # Load train embeddings + embed val
    X_train = load_train_embeddings()
    X_val   = embed_val(val_df["description"].tolist())

    # Load training labels (needed to re-fit tuned models on full train set)
    ...
    lines = []  # lines for the report file
    ...
    cause_report = classification_report(
        y_cause_val, best_cause_clf.predict(X_val),
    ...
    prio_report = classification_report(
        y_priority_val, best_prio_clf.predict(X_val),
