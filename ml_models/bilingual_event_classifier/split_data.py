...
split_data.py
=============
Reads data/processed/classifier_events_clean.csv and writes:
    data/splits/train.csv
    data/splits/val.csv
    data/splits/test.csv

WHY group-aware split (not random):
    ~24% of descriptions are exact or near-exact duplicates
    ("Starting problem" appears hundreds of times).  A naive random
    split would put the same sentence in train AND test, making the
    model look better than it is — classic data leakage.

    Fix: group rows by their exact description text, then split at the
    GROUP level so each unique sentence lands in exactly one split.

Split: 70% train / 15% val / 15% test, stratified by event_cause.
...
def group_stratified_split(df, label_col, group_col, train_r, val_r, seed):
    ...
    group_label = df.groupby(group_col)[label_col].agg(
        lambda x: x.mode()[0]
    ).to_dict()
...
def main():
    ...
    print(f"[split_data] Unique description groups: {n_groups:,} (out of {len(df):,} rows)")
