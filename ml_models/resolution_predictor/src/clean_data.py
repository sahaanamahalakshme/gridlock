...
# Columns we will use for training + the target
FEATURE_COLS = [
    "event_cause",
    "corridor",
    "priority",
...
def parse_datetimes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Use resolved_datetime first; fall back to closed_datetime
...
def filter_usable(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep only unplanned (planned events → Person C's model)
    df = df[df["event_type"] == "unplanned"].copy()
    print(f"[clean] After keeping unplanned only: {len(df)} rows")

    # Keep rows with a valid positive duration under 10 hours
    ...
    # Drop rows missing any feature we will train on
    core_features = ["event_cause", "corridor", "priority", "requires_road_closure", "police_station"]
    df = df.dropna(subset=core_features).copy()
    print(f"[clean] After dropping rows with missing core features: {len(df)} rows  (dropped {before - len(df)} total)")
    return df


def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    df["hour_of_day"] = df["start"].dt.hour
    df["day_of_week"] = df["start"].dt.dayofweek   # 0 = Monday

    # Peak hours: 7-10 AM and 5-8 PM
    df["is_peak_hour"] = df["hour_of_day"].isin(range(7, 10)).astype(int) | \
                         df["hour_of_day"].isin(range(17, 20)).astype(int)
    df["is_peak_hour"] = df["is_peak_hour"].astype(int)

    # Normalise requires_road_closure to int 0/1
