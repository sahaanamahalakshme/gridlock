...
def main():
    ...
    if rare_causes:
        ...
        print(
            f"[split] Rare causes (< 10 rows) bucketed as '__rare__' for stratification: {rare_causes }"
        )
    ...
    if temp_counts.min() >= 2:
        ...
        print(
            "[split] Temp split too small to stratify — splitting without stratification"
        )
    ...
    print(
        f"[split] train : {len (train_df )} rows  ({len (train_df )/len (df )*100 :.1f}%)"
    )

    print(
        f"[split] val   : {len (val_df )} rows  ({len (val_df )/len (df )*100 :.1f}%)"
    )

    print(
        f"[split] test  : {len (test_df )} rows  ({len (test_df )/len (df )*100 :.1f}%)"
    )
