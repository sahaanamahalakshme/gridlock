...
def main():
    ...
    grouped = valid.groupby(["corridor", "hour"]).size().reset_index(name="total_events")
    ...
    out = {"n_days": n_days, "thresholds": THRESHOLDS, "corridors": baseline, "system": system}
    ...
    print(f"[baseline] System peak hour: {max(system, key=lambda h: system[h]['avg_per_day'])}:00")
