...
BASELINE_PATH = os.path.join(os.path.dirname(__file__), "..", "output", "baseline.json")
...
def load_baseline(path: str = BASELINE_PATH) -> dict:
    ...
    print(
        f"[baseline] Loaded. {n_corridors} corridors, {data['n_days']} days of history."
    )
    return data
...
def score_spike(
    ...
    hour_data = corr_data.get(
        str(current_hour),
        corr_data.get(str(current_hour), corr_data.get(current_hour, None)),
    )

    if hour_data is None:

        sys_hour = system.get(
            str(current_hour), system.get(current_hour, {"avg_per_day": 0.0})
        )
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        (
            ...
            12,
            1,
            "Mysore Road at quiet midday with 1 event → expect severe",
        ),
    ...
    for corridor, hour, count, desc in test_cases:
        ...
        print(f"  ── {desc}")
        print(
            f"     spike_ratio={result['spike_ratio']}  label={result['spike_label']}  "
