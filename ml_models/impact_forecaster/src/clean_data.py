...
KEEP_COLS = [
    "id", "description", "event_cause", "corridor", "priority",
    "requires_road_closure", "address", "latitude", "longitude",
...
def clean():
    ...
    planned["start_datetime"] = pd.to_datetime(planned["start_datetime"], errors="coerce")
    planned["end_datetime"] = pd.to_datetime(planned["end_datetime"], errors="coerce")

    duration = (planned["end_datetime"] - planned["start_datetime"]).dt.total_seconds() / 60
    ...
    print(f"rows with usable duration_minutes: {planned['duration_minutes'].notna().sum()}")
    return planned
