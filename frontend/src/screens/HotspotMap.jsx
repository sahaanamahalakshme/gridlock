...
function HotspotMapInner({ searchQuery, onPredictClick, apiData }) {
  ...
  const peakJuncText = peakJunc
    ? `${peakJunc.name.replace(" Junction", "")} (${peakJunc.count})`
    : "N/A";
  const mostCommonCause =
    Object.entries(CAUSE_TOTALS).sort((a, b) => b[1] - a[1])[0]?.[0] ||
    "others";
  const totalEvents = apiData.junctions.reduce((sum, j) => sum + j.count, 0);
  const avgEvents =
    totalJunctions > 0 ? Math.round(totalEvents / totalJunctions) : 0;
  const chronicLocationsCount = apiData.junctions.filter(
    (j) => j.is_hotspot,
  ).length;
    ...
    map.on("load", () => {
      ...
      junctionsWithHourly.forEach((j, i) => {
        ...
        el.style.backgroundColor =
          colors.causes[j.dominant_cause] || colors.causes.others;

        if (j.is_hotspot) {
          el.style.border = "2px dashed #EF4444";
          el.style.boxShadow = "0 0 8px rgba(239, 68, 68, 0.8)";
        } else {
        ...
        el.addEventListener("click", () => {
          ...
          features.forEach((f) =>
            map.setFeatureState(
              { source: "hotspots", id: f.id },
              { selected: false },
            ),
          );
          map.setFeatureState(
            { source: "hotspots", id: i },
            { selected: true },
          );
        });
  ...
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !map.isStyleLoaded() || !map.getLayer("hotspots-extrusion"))
      return;
    ...
    map.setPaintProperty("hotspots-extrusion", "fill-extrusion-color", [
      "case",
      ["!=", ["index-of", q, ["downcase", ["to-string", ["get", "name"]]]], -1],
      ["get", "color"],
      [
        "!=",
        ["index-of", q, ["downcase", ["to-string", ["get", "dominant_cause"]]]],
        -1,
      ],
      ["get", "color"],
      "#888888",
    ...
    junctionsWithHourly.forEach((j, i) => {
      ...
      if (el) {
        const isMatch =
          ...
          j.dominant_cause.toLowerCase().includes(q);
      ...
      >
        <div>
          ...
          <p style={{ ...typography.subtitle, margin: 0, marginTop: "2px" }}>
            Historical event density by junction · Bengaluru
          </p>
        </div>
        <div
          style={{
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              ...
              {selectedJunction && (
                <>
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        padding: "4px 10px",
                        borderRadius: "99px",
                        backgroundColor: `${colors.causes[selectedJunction.dominant_cause]}15`,
                        color: colors.causes[selectedJunction.dominant_cause],
                        border: `1px solid ${colors.causes[selectedJunction.dominant_cause]}30`,
                      }}
                    >
                      {selectedJunction.dominant_cause.replace("_", " ")}
                    </span>
                    {selectedJunction.is_hotspot && (
                      <span
                        style={{
                          fontSize: "11px",
                          fontWeight: 600,
                      ...
                      >
                        🔴 CHRONIC:{" "}
                        {selectedJunction.hotspot_tier?.toUpperCase()}
                      </span>
                    )}
                  </div>

                  {selectedJunction.is_hotspot &&
                    selectedJunction.route_to &&
                    selectedJunction.route_to !== "Traffic Police" && (
                      <div
                        style={{
                          backgroundColor: "#EFF6FF",
                      ...
                      >
                        ...
                        <div>
                          ...
                          Based on chronic issue history, incidents here often
                          route to <strong>{selectedJunction.route_to}</strong>.
                        </div>
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    <div style={{ ...typography.value, fontSize: "16px" }}>
                      54 min avg
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div style={{ ...typography.body }}>
                      {selectedJunction.name.split(" ")[0]} Traffic PS
                    </div>
                  </div>

                  <div>
                    <div style={{ ...typography.label, marginBottom: "12px" }}>
                      Top Event Causes
                    </div>
                    <div
                      style={{
                    ...
                    >
                      {["vehicle_breakdown", "accident", "water_logging"].map(
                        (cause, idx) => {
                          const val = Math.round(
                            selectedJunction.count * (1 - idx * 0.3),
                          );
                  ...
                  <div style={{ marginTop: "auto", paddingTop: "20px" }}>
                    <button
                      ...
                      onClick={() =>
                        onPredictClick({
                          corridor: "CBD 1",
                          policeStation: selectedJunction.name.split(" ")[0],
                        })
                      }
                    >
          ...
          >
            ...
            >
              <span style={{ ...typography.label, width: "80px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div style={cards.base}>
            ...
            >
              ...
              <div>
                <div style={typography.label}>Chronic locations</div>
                <div
                  style={{
                    ...typography.value,
                    fontSize: "18px",
