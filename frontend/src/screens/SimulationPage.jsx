    ...
    >
      ...
      <span
        style={{
          ...
          color: active ? "#2563EB" : "var(--color-text-tertiary)",
          fontWeight: 600,
          whiteSpace: "nowrap",
        }}
      >
  ...
  return (
    <div
      style={{
        ...
        height: "2px",
        marginBottom: "20px",
        backgroundColor: done ? "#059669" : "var(--color-border)",
      ...
      {COMPASS_DIRS.map(({ label, bearing, top, left, transform }) => (
        <button
          ...
          style={{
            ...
            backgroundColor:
              value === bearing ? "#DC2626" : "var(--color-card-bg)",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
      ...
      {classification && (
        <div style={{ ...cardStyle }}>
          ...
          >
            <span
              style={tagStyle(
                CAUSE_COLORS[classification.event_cause] || "#6B7280",
              )}
            >
            ...
            >
              {Math.round((classification.cause_confidence || 0) * 100)}%
              confidence
            </span>
          </div>
        </div>
      ...
      <div style={{ ...cardStyle }}>
        <div style={labelStyle}>
          Directional traffic plan — Rally moving {rally_direction_label}
        </div>
        <div
          style={{
            display: "flex",
...
export default function SimulationPage() {
  ...
  const [mode, setMode] = useState("unplanned");
  ...
  useEffect(() => {
    ...
    const onClick = async (e) => {
      ...
      try {
        ...
        const data = await res.json();
        address =
          data.display_name?.split(",").slice(0, 3).join(", ") || address;
      } catch (_) {}
  ...
  const clearMapState = useCallback(() => {
    ...
    const layerIds = [
      ...
      "dir-head_on-border",
      "dir-tail_on-border",
      "dir-cross_left-border",
      "dir-cross_right-border",
      "dir-head_on-approach",
      "dir-tail_on-approach",
      "dir-cross_left-approach",
      "dir-cross_right-approach",
      "dir-head_on-approach-border",
      "dir-tail_on-approach-border",
      "dir-cross_left-approach-border",
      "dir-cross_right-approach-border",
    ];
    layerIds.forEach((id) => {
  ...
  const upsertLineLayer = useCallback(
    (map, id, coords, color, width, dasharray, opacity) => {
      const geojson = {
        type: "Feature",
  ...
  const drawDirectionalResult = useCallback(
    async (data, rallyBearing, incidentLat, incidentLng) => {
      const map = mapRef.current;
      if (!map) return;
      ...
      for (let i = 0; i <= 48; i++) {
        ...
        circlePts.push([
          incidentLng + rDeg * Math.cos(a),
          incidentLat + rDeg * 0.9 * Math.sin(a),
        ]);
      }
      if (map.getSource("impact-circle")) {
        map
          .getSource("impact-circle")
          .setData({
            type: "Feature",
            geometry: { type: "Polygon", coordinates: [circlePts] },
          });
      } else {
        map.addSource("impact-circle", {
          type: "geojson",
          data: {
            type: "Feature",
            geometry: { type: "Polygon", coordinates: [circlePts] },
      ...
      {
        const A = off(incidentLat, incidentLng, F, D);
        placeDiversionPin(A[0], A[1], "#DC2626", "⚠️", "Head-on");
        allPts.push(A);
      }

      {
        const A = off(incidentLat, incidentLng, B, D);
        placeDiversionPin(A[0], A[1], "#D97706", "🔁", "Tail-on");
        allPts.push(A);
      }

      {
        const A = off(incidentLat, incidentLng, L, D);
        placeDiversionPin(A[0], A[1], "#2563EB", "↰", "Cross L");
        allPts.push(A);
      }

      {
        const A = off(incidentLat, incidentLng, R, D);
  ...
  const drawResult = useCallback(
    (data, isPlanned) => {
      ...
      placeMarker(
        data.barricade_point[0],
        data.barricade_point[1],
        "barricade",
      );
      ...
      if (data.diversions?.length > 0) {
        const arrowCoords = data.diversions
          .filter((d) => d.centroid)
          .map((d) => [
        ...
        if (arrowCoords.length > 0) {
          const geojson = {
            type: "Feature",
            geometry: { type: "MultiLineString", coordinates: arrowCoords },
          };
          if (map.getSource("diversion-arrow")) {
            map.getSource("diversion-arrow").setData(geojson);
          } else {
            map.addSource("diversion-arrow", {
              type: "geojson",
              data: geojson,
            });
            map.addLayer({
      ...
      const allPoints = [
        data.barricade_point,
        ...(data.diversions || []).map((d) => d.centroid).filter(Boolean),
      ];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      >
        <div>
          <h1 style={{ ...typography.header, margin: 0, fontSize: "16px" }}>
            Traffic Diversion Simulator
          </h1>
          <p style={{ ...typography.subtitle, margin: 0, marginTop: "2px" }}>
            Simulate planned or unplanned events · See recommended diversion in
            real time
          </p>
        </div>
        <button
      ...
      <div style={{ ...cards.base, padding: "16px 24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <StepDot
            num={2}
            label={mode === "unplanned" ? "Pin incident" : "Pin route"}
            active={step === 1}
            done={step > 1}
          />
            ...
            >
                  ...
                  ? [
                      {
                        color: "#F97316",
                        emoji: "🚧",
                        label: "Barricade / Incident",
                      },
                      {
                        color: "#111827",
                        emoji: "━",
                        label: "Rally route (black)",
                      },
                      {
                        color: "#DC2626",
                        emoji: "━",
                        label: "Head-on diversion",
                      },
                      {
                        color: "#D97706",
                        emoji: "━",
                        label: "Tail-on diversion",
                      },
                      {
                        color: "#2563EB",
                        emoji: "━",
                        label: "Cross-left diversion",
                      },
                      {
                        color: "#7C3AED",
                        emoji: "━",
                        label: "Cross-right diversion",
                      },
                    ]
                  : [
                      {
                      ...
                      },
                      {
                        color: "#059669",
                        emoji: "●",
                        label: "Diversion corridor",
                      },
                      {
                        color: "#2563EB",
                        emoji: "—",
                        label: "Event path (planned)",
                      },
                      {
                        color: "#94A3B8",
                        emoji: "- -",
                        label: "Corridors (overlay)",
                      },
                    ]
              ...
              {clickStep > 0 && (
                ...
                >
                  {clickStep === 1
                    ? mode === "unplanned"
                      ? "📍 Click where the incident occurred"
                      : "📍 Click the event start point"
                ...
                >
                  <span
                    style={{
                      ...
                      color:
                        CAUSE_COLORS[result.classification?.event_cause] ||
                        "#6B7280",
                  ...
                  >
                    {Math.round(
                      (result.classification?.cause_confidence || 0) * 100,
                    )}
                    % confidence
                  </span>
                </div>
              ...
              <div style={cards.base}>
                ...
                <div style={{ ...typography.value, fontSize: "14px" }}>
                  {result.affected_corridor ||
                    result.start_corridor ||
                    "Not identified"}
                </div>
                {result.start_corridor &&
                  result.end_corridor &&
                  result.end_corridor !== result.start_corridor && (
                    <div
                      style={{
                        marginTop: "4px",
                        fontSize: "12px",
                  ...
                  {result.diversions.map((d, i) => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: "6px",
                        marginBottom:
                          i < result.diversions.length - 1 ? "8px" : 0,
                        backgroundColor:
                          i === 0 ? "#F0FDF4" : "var(--color-gray-bg)",
                      ...
                      >
                        <span
                          style={{
                            ...
                            fontWeight: 600,
                            color:
                              i === 0 ? "#059669" : "var(--color-text-primary)",
                          }}
                        >
                          {i === 0 ? "✓ " : `${i + 1}. `}
                          {d.corridor}
              ...
              {result.resolution_estimate?.predicted_minutes && (
                <div style={cards.base}>
                  ...
                  <div style={{ ...typography.value, fontSize: "20px" }}>
                    ~{Math.round(result.resolution_estimate.predicted_minutes)}{" "}
                    min
                  </div>
                  {result.resolution_estimate.confidence_band && (
                    ...
                    >
                      Range:{" "}
                      {Math.round(
                        result.resolution_estimate.confidence_band[0],
                      )}
                      –
                      {Math.round(
                        result.resolution_estimate.confidence_band[1],
                      )}{" "}
                      min
                    </div>
                  )}
                ...
                <span>
                  {startPoint ? (
                    <span style={{ color: "inherit" }}>
                      {startPoint.address.substring(0, 38)}…
                    </span>
                  ) : mode === "unplanned" ? (
                    "Click map to pin incident"
                  ) : (
                    "Click map to pin start point"
                  )}
                </span>
              </button>
